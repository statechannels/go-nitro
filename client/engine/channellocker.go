...
func (l *ChannelLocker) Lock(objective protocols.Objective) {
	...
	channelIds := getChannelIds(objective.Related())
...
func getChannelIds(related []protocols.Storable) []types.Destination {
	channelIds := make([]types.Destination, 0)
	for _, storable := range related {
		switch c := storable.(type) {
