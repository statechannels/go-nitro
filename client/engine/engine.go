...
type Engine struct {
	...
	AppRequestsFromAPI       chan *types.AppRequest
...
func New(msg messageservice.MessageService, chain chainservice.ChainService, store store.Store, logDestination io.Writer, policymaker PolicyMaker, metricsApi MetricsApi) Engine {
	...
	e.AppRequestsFromAPI = make(chan *types.AppRequest, 100)
	...
	e.Am = app.NewAppManager(e.logger, e.store)
...
func (e *Engine) Run() {
	for {
		...
		e.metrics.RecordQueueLength("api_virtual_app_request_queue", len(e.AppRequestsFromAPI))
		...
		case appRequest := <-e.AppRequestsFromAPI:
			err = e.Am.HandleRequest(appRequest)
...
func (e *Engine) handleMessage(message protocols.Message) (EngineEvent, error) {
	...
	for _, ar := range message.AppRequests {
		err := e.Am.HandleRequest(&ar)
		if err != nil {
			return EngineEvent{}, err
		}
	}
