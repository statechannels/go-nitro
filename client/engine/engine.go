...
func New(msg messageservice.MessageService, chain chainservice.ChainService, store store.Store, logDestination io.Writer, policymaker PolicyMaker, metricsApi MetricsApi) Engine {
	...
	e.ObjectiveRequestsFromAPI = make(chan protocols.ObjectiveRequest, 1_000_000)
	e.PaymentRequestsFromAPI = make(chan PaymentRequest, 1_000_000)
	...
	e.toApi = make(chan EngineEvent, 1_000_000)
