...
func (ms *P2PMessageService) gossipStreamHandler(stream network.Stream) {
	reader := bufio.NewReader(stream)
	// Create a buffer stream for non blocking read and write.
	raw, err := reader.ReadString(DELIMITER)
...
func (ms *P2PMessageService) gossip(ctx context.Context) {
	for {
		select {
		case <-ctx.Done():
			return
		case <-time.After(PEER_EXCHANGE_SLEEP_DURATION):

			for _, p := range ms.p2pHost.Peerstore().Peers() {
				alreadyMessaged, _ := ms.gossipedTo.Load(p.String())
				if p != ms.Id() && !alreadyMessaged {
					ms.gossipedTo.Store(p.String(), true)
					ms.sendInfoToPeer(p)
				}
			}
		}
