...
describe('Benchmark tests', () => {
  ...
  describe('Consumes the expected gas for happy-path exits', () => {
    it(`when exiting a directly funded (with ETH) channel`, async () => {
      // begin setup
      await (
        await nitroAdjudicator.deposit(MAGIC_ADDRESS_INDICATING_ETH, X.channelId, 0, 10, {
          value: 10,
        })
      ).wait();
      // end setup
      ...
      ).toConsumeGas(gasRequiredTo.ETHexit.satp);
    });
  ...
  describe('Consumes the expected gas for sad-path exits', () => {
    ...
    it(`when exiting a ledger funded (with ETH) channel`, async () => {
      ...
      // end setup
      // initially                   ⬛ ->  L  ->  X  -> 👩
      const {proof: ledgerProof, finalizesAt: ledgerFinalizesAt} =
