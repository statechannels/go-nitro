...
import CurrencySwapAppArtifact from '../../../artifacts/contracts/CurrencySwapApp.sol/CurrencySwapApp.json';
import {
...
beforeAll(async () => {
  CurrencySwapApp = setupContract(
    provider,
    CurrencySwapAppArtifact,
...
describe('requireStateSupported (lone candidate route)', () => {
  ...
  testcases.map(async tc => {
    it(`${tc.reason ? 'reverts        ' : 'does not revert'} for unaninmous consensus on ${
      ...
      if (tc.reason) {
        await expectRevert(
          () => CurrencySwapApp.requireStateSupported(fixedPart, [], candidate),
          tc.reason
        );
      } else {
        await CurrencySwapApp.requireStateSupported(fixedPart, [], candidate);
      }
    });
  });
});
...
describe('requireStateSupported (candidate plus single proof state route)', () => {
  ...
  testcases.map(async tc => {
    it(`${
      ...
      if (tc.reason) {
        await expectRevert(
          () => CurrencySwapApp.requireStateSupported(fixedPart, proof, candidate),
          tc.reason
        );
      } else {
        await CurrencySwapApp.requireStateSupported(fixedPart, proof, candidate);
      }
    });
  });
});
...
describe('requireStateSupported (longer proof state route)', () => {
  it(`reverts for |support|>1`, async () => {
    ...
    await expectRevert(
      () => CurrencySwapApp.requireStateSupported(fixedPart, [candidate, candidate], candidate),
      'bad proof length'
    );
  });
});
