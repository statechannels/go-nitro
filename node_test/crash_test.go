...
func TestCrashTolerance(t *testing.T) {
	...
	nodeA := node.New(messageserviceA, chainA, storeA, []policy.PolicyMaker{policy.NewFairOutcomePolicy(ta.Alice.Address())})
	...
	// test successful condition for setup / teardown of unused ledger channel
	{
		...
		anotherClientA := node.New(
			...
			anotherChainA,
			anotherStoreA, []policy.PolicyMaker{policy.NewFairOutcomePolicy(ta.Alice.Address())})
