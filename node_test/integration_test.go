...
func RunIntegrationTestCase(tc TestCase, t *testing.T) {
	...
	t.Run(tc.Description, func(t *testing.T) {
		...
		for i, clientI := range intermediaries {
			...
			aliceLedgers[i] = openLedgerChannel(t, infra.simulatedChain, clientA, clientI, asset)
			...
			bobLedgers[i] = openLedgerChannel(t, infra.simulatedChain, clientI, clientB, asset)
			checkLedgerChannel(t, bobLedgers[i], initialLedgerOutcome(*clientI.Address, *clientB.Address, asset), query.Open, clientB)

		}

		if tc.NumOfHops == 2 {
			openLedgerChannel(t, infra.simulatedChain, intermediaries[0], intermediaries[1], asset)
		}
